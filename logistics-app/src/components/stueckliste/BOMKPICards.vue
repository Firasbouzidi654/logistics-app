...
<template>
  <div class="grid grid-cols-2 lg:grid-cols-6 gap-4">
    ...
    <!-- Critical components + risk level -->
    <div class="glass rounded-2xl border border-rose-500/25 bg-gradient-to-br from-rose-600/15 to-orange-700/10 p-5">
      <div class="flex items-center gap-2 mb-2">
        <div class="w-7 h-7 rounded-lg bg-rose-500/20 flex items-center justify-center text-rose-400 font-bold">!</div>
        <span class="text-xs text-slate-400 font-medium uppercase tracking-wider">Kritische Komponenten</span>
      </div>
      <p class="text-2xl font-black text-white">{{ store.criticalComponents }}</p>
      <p class="text-slate-500 text-xs mt-1">Niedrig + nicht vorrätig</p>
      <div :class="['mt-3 rounded-lg border px-2.5 py-2', store.riskLevel.background]">
        <p class="text-[11px] uppercase tracking-wider text-slate-500 font-semibold">Risiko-Level</p>
        <p :class="['text-xs font-bold mt-0.5', store.riskLevel.tone]">{{ store.riskLevel.label }}</p>
      </div>
    </div>
